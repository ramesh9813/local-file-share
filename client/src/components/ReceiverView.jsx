...
export default function ReceiverView({ 
  ...
  const { allActiveSessions = [], refreshActiveSessions, markSessionUnlocked } = useSessions();
  ...
  const handleConnect = async (customCode) => {
    ...
    try {
      ...
      if (verifyRes && verifyRes.success && verifyRes.room) {
        ...
        if (typeof markSessionUnlocked === 'function') {
          markSessionUnlocked(verifyRes.room);
        }
                    ...
                    {s.isReceiverConnected ? (
                      ...
                      <>
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm transition"
                        >
                          View Files
                        </button>
                      </>
                    ) : (
                      /* NOT CONNECTED YET: Show PIN Protected and Enter PIN button */
                      <>
